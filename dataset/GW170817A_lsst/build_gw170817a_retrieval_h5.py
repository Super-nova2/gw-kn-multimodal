#!/usr/bin/env python3
"""Build an ALBEF-compatible retrieval HDF5 for the GW170817A LSST experiment."""
...
DEFAULT_SNANA_SIM_DIR = f"/fred/oz016/bgao_kn/SNANA/SNDATA_ROOT/SIM/{DEFAULT_GENVERSION}"
DEFAULT_SKYMAP = str(GW170817A_DIR / "bayestar_no_virgo.fits")
DEFAULT_SCALAR_PREFIX = np.asarray([1.46, 1.27, 0.0, 0.0, 0.7959, 0.0, 0.0], dtype=np.float32)
...
def _format_lightcurve(
    *,
    ...
    rel_times = (np.asarray(lc_mjd, dtype=np.float64) - float(event_time_mjd)) / 100.0
    ...
    if not np.any(masks > 0):
        return None
    return values, errors, masks, times


def resolve_snana_fits(sim_dir: str | Path, genversion: str = DEFAULT_GENVERSION) -> tuple[Path, Path]:
    root = Path(sim_dir)
    head = root / f"{genversion}_HEAD.FITS"
    phot = root / f"{genversion}_PHOT.FITS"
    if not head.exists() and head.with_suffix(head.suffix + ".gz").exists():
        head = head.with_suffix(head.suffix + ".gz")
    if not phot.exists() and phot.with_suffix(phot.suffix + ".gz").exists():
        phot = phot.with_suffix(phot.suffix + ".gz")
    if not head.exists() or not phot.exists():
        raise FileNotFoundError(f"Could not find HEAD/PHOT FITS under {root} for {genversion}")
    return head, phot
    ...
    *,
    ...
    with fits.open(head_path) as hdul_head, fits.open(phot_path) as hdul_phot:
        ...
        for row in head:
            ...
            values, errors, masks, times = formatted
            records.append(
...
def write_retrieval_h5(
    ...
    *,
    ...
    with h5py.File(output, "w") as f:
        ...
        opt.create_dataset("zero_time_mjd_base", data=np.asarray([r.event_time_mjd for r in records], dtype=np.float64), chunks=(opt_chunk,), maxshape=(None,))
        ...
        f.attrs["dataset_mode"] = "gw170817a_lsst_redshift_test"
        ...
        f.attrs["time_zero_base_semantics"] = "optical zero_time_mjd_base stores SNANA SIM_MJD_EXPLODE"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    head, phot = resolve_snana_fits(args.sim_dir, args.genversion)
