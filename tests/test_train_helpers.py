...
class GalleryHardConfigTests(unittest.TestCase):
    def test_default_json_includes_hard_mining_params(self):
        import json
        defaults_path = os.path.join(
            MODEL_DIR, "args", "defaults",
            "ALBEF_BNS_NSBH_v9_default.json",
        )
        with open(defaults_path) as f:
            cfg = json.load(f)
        self.assertTrue(cfg.get("gallery_hard_neg_enable", False))
        self.assertEqual(cfg.get("gallery_hard_neg_topk"), 32)
        self.assertEqual(cfg.get("gallery_hard_neg_weight"), 0.5)
...
class GallerySafetyConfigTests(unittest.TestCase):
    ...
    def test_v10_default_json_includes_gallery_safety_defaults(self):
        cfg = self._load_json(
            "args",
            "defaults",
            "ALBEF_BNS_NSBH_v10_default.json",
        )

        self.assertFalse(cfg.get("gallery_force_include_positives"))
        self.assertTrue(cfg.get("gallery_detach_encoder_inputs"))
        self.assertEqual(cfg.get("gallery_loss_ramp_epochs"), 5)

    def test_v10_run_config_uses_safe_gallery_weight_and_rerank_lambda(self):
        active_v10 = os.path.join(
            os.path.dirname(__file__), "..", "args", "ALBEF_BNS_NSBH_v10.json"
        )
        if not os.path.exists(active_v10):
            self.skipTest("active v10 run config is not present in this checkout")
        cfg = self._load_json("args", "ALBEF_BNS_NSBH_v10.json")

        self.assertAlmostEqual(cfg.get("gallery_loss_weight"), 0.05)
        self.assertAlmostEqual(cfg.get("fusion_rerank_lambda"), 0.5)
        self.assertFalse(cfg.get("gallery_force_include_positives"))
        self.assertTrue(cfg.get("gallery_detach_encoder_inputs"))
        self.assertEqual(cfg.get("gallery_loss_ramp_epochs"), 5)
