...
class TimeZeroRegressionTests(unittest.TestCase):
    ...
    def test_optical_only_launcher_does_not_forward_offset_flags(self):
        script = (REPO_ROOT / "optical_only" / "train_optical_only.sh").read_text()

        self.assertNotIn("cmd+=(--time_offset_enable)", script)
        self.assertNotIn("eval_cmd+=(--time_offset_enable)", script)
        self.assertNotIn("cmd+=(--offset_", script)
        self.assertNotIn("eval_cmd+=(--offset_", script)
