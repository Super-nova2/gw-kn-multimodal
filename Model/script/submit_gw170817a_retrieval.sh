...
DEFAULT_CONFIG_REL="Model/args/eval/retrieval_gw170817a_lsst_v11.json"
...
EVAL_SCRIPT="${MODEL_DIR}/script/eval_gw170817a_retrieval.py"
DEFAULT_CONFIG_PATH="${REPO_ROOT}/${DEFAULT_CONFIG_REL}"
DEFAULT_OUTPUT_DIR="${MODEL_DIR}/eval_results/gw170817a_lsst_retrieval"
...
if [[ ! -f "${config_file}" ]]; then
    ...
    echo "Usage: $0 [retrieval_gw170817a_lsst.json]" >&2
    exit 1
fi
...
if ! command -v jq >/dev/null 2>&1; then
    echo "jq not found; required to parse GW170817A retrieval config." >&2
    exit 1
fi
...
if [[ ! -f "$TEST_DATA_PATH" ]]; then
    echo "GW170817A retrieval HDF5 not found: $TEST_DATA_PATH" >&2
    exit 1
fi
...
echo "========================================"
echo "GW170817A Retrieval Evaluation"
echo "========================================"
...
echo "Candidate time window: +/-${GALLERY_CANDIDATE_TIME_WINDOW_DAYS:-50} days"
echo "Candidate credible max: ${GALLERY_CANDIDATE_CREDIBLE_LEVEL_MAX:-0.9}"
...
if [[ ${exit_code} -ne 0 ]]; then
    echo "GW170817A retrieval evaluation failed with exit code ${exit_code}" >&2
    exit ${exit_code}
fi
